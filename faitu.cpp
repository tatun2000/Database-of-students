...
void SecondWindow::Show_FAITU_Window()
{
    SecondWindow::show();
}
    ...
    ui(new Ui::SecondWindow)
{
    ...
    // Request to the database for displaying groups of the selected faculty
    obj.exec("SELECT number_of_group FROM All_Groups WHERE id_facult = 1;");
    while(obj.next())
    {
        // Output a list of groups in Qlistwidget
        ui->listWidget->addItem(obj.value(0).toString() + " " + obj.value(1).toString());
    }
    Initial_Settings();
}
...
void SecondWindow::Print_Info_About_Of_Students()
{
    ...
    else
    {
        ...
        {
            // Displaying information about students in QtableWidget
    ...
    // Counting the number of students in a group
...
{
    SecondWindow::close();
    emit FAITU_Window_Close(); // Signal to close the form
}

QStringList SecondWindow::Inizialisation_Titles_Of_Tables()
{
    QStringList value;
...
void SecondWindow::Initial_Settings()
{
    qtw0 = nullptr;
    qtw1 = nullptr;
    qtw2 = nullptr;
    ...
    ui->tableWidget->horizontalHeader()->setDefaultSectionSize(250); // Standard column size
...
void SecondWindow::on_listWidget_itemClicked()
{
    emit signal_about_change(); // Signal for switching between groups
    ui->pushButton_3->setVisible(true);
    ui->pushButton_4->setVisible(true);
    ui->pushButton_2->setVisible(false);
    ui->comboBox->setVisible(false);
...
void SecondWindow::on_pushButton_3_clicked()
{
    ...
    // Request to the database to display the ID of students in QComboBox
    ...
    else
    {
        while(obj.next())
        {
            // Output a list of students in QComboBox
            ui->comboBox->addItem(obj.value(0).toString());
        }
    }
}
...
void SecondWindow::on_pushButton_2_clicked()
{
    ...
    // Removing the student group from QListWidget
    // and with the selected ID from QComboBox from the database
...
void SecondWindow::Counting_The_Number_Of_Students_In_A_Group()
{
    int key = ui->listWidget->currentItem()->text().toInt();
    QString default_values = "0";
    QString values = map_with_number_of_groups.value(key,default_values); // id group

    // Request to the database to count the number of students in the group
